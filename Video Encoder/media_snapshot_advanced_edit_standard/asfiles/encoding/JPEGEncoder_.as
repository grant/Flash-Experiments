	...
	public class JPEGEncoder
	{
		...
		private function fDCTQuant(data:Array, fdtbl:Array):Array
		{
			...
			for (var j:int=0; j<8; j++) {
				...
				/* The rotator is modified from fig 4-8 to avoid extra negations. */
				var z5p2:Number = (tmp10 - tmp12) * 0.382683433; /* c6 */
				var z2p2:Number = 0.541196100 * tmp10 + z5p2; /* c2-c6 */
				var z4p2:Number = 1.306562965 * tmp12 + z5p2; /* c2+c6 */
			...
			for (var k:int=0; k<64; k++) {
				...
				data[k] = Math.round((data[i]*fdtbl[k]));
			}
			return data;
		}
		...
		private function processDU(CDU:Array, fdtbl:Array, DC:Number, HTDC:Array, HTAC:Array):Number
		{
			...
			while ( j <= end0pos ) {
				var startpos:int = j;
				for (; (DU[i]==0) && (j<=end0pos); j++) {
				}
				var nrzeroes:int = j-startpos;
				if ( nrzeroes >= 16 ) {
				...
				writeBits(HTAC[nrzeroes*16+category[32767+DU[j]]]);
